...
CloakRFQ Receipts needs an MVP implementation plan that preserves real-world RFQ functionality while demonstrating Canton-native selective disclosure, role-based authorization, private quote handling, funding-backed quote eligibility, Seller-controlled quote selection, fallback, On-Ledger Demo Settlement, and scoped settlement evidence.

## Solution

Build CloakRFQ Receipts as a private, functionality-preserving RFQ marketplace for Receivable Sales on Canton.

The MVP demonstrates one complete happy path and one atomic failed-settlement recovery path. A Seller creates a Receivable, opens a Blind RFQ, receives Eligible Quotes through a Seller Quote View, selects a Quote, and completes On-Ledger Demo Settlement using a committed CIP-56 token allocation. Funders submit Private Quotes backed by funding evidence, concretely a committed CIP-56 allocation in Phase 2 and do not see each other's quotes. Compliance and optional Risk parties provide scoped attestations. If settlement fails, the Seller sees the ledger failure reason and may retry or choose another still-valid Quote. After successful settlement, the designated Auditor receives scoped `ReceivableSaleSettlement` evidence rather than the full RFQ workflow.
...
- Seller can select a Quote using Selection Criteria.
- Selected Quote can either settle or fail before RFQ Finality.
- Seller can retry a failed settlement or choose another still-valid Quote.
- On-Ledger Demo Settlement pays the Seller and initiates the pending Receivable transfer; the Winning Funder accepts ownership afterward.
- The designated Auditor receives scoped `ReceivableSaleSettlement` evidence without full RFQ visibility.
...
16. As a Seller, I want to select a Quote based on Selection Criteria, so that I am not forced to select the highest headline price.
...
22. As a Seller, I want to retry after a failed settlement or choose another still-valid Quote, so that one failed attempt does not end the RFQ.
23. As a Seller, I want the ledger failure reason surfaced so that I can retry or choose another still-valid Quote.
24. As a Seller, I want successful settlement to end further use of the settled Quote, so that failure recovery is not confused with post-settlement Funder exit.
...
45. As an Auditor, I want to receive a `ReceivableSaleSettlement` record, so that I can verify required workflow outcomes without seeing the full RFQ.
...
47. As the designated Auditor, I want `ReceivableSaleSettlement` to identify the Seller, Winning Funder, accepted price, payment and transfer references, and settlement time, so that I can inspect scoped outcome evidence.
...
- Allow the Seller to select a Quote using Selection Criteria rather than highest headline price alone.
...
- After a failed settlement, surface the ledger error and allow the Seller to retry or select another still-valid Quote.
...
- Create `ReceivableSaleSettlement` evidence for the designated Auditor after successful settlement.
...
- Keep the MVP focused on one complete happy path and one atomic failed-settlement recovery path.
- Track 1 positioning is Private DeFi & Capital Markets, with private credit / invoice financing as the use case.
- Post-MVP boundaries include production-grade funding integration, jurisdiction-specific Receivable assignment, Debtor identity and notification rules, post-settlement Funder exit, penalties or reputation, and production payment integration.

## Testing Decisions

- The primary testing seam should be the full Daml/Canton workflow lifecycle, tested at the highest practical level: happy path plus failed-settlement recovery across role-specific party views.
...
- Happy path coverage should verify Receivable creation, Blind RFQ opening, optional Risk Attestation, Compliance Attestation, RFQ Disclosure Package delivery, Private Quote submission, funding evidence status, Seller Quote View, Quote selection, Settlement Window entry, On-Ledger Demo Settlement, and `ReceivableSaleSettlement` record creation.
- Failure-path coverage should verify that settlement failure is atomic, the ledger error reaches the Seller UI, and a still-valid Quote remains available for retry or selection.
- Role-view tests should verify that competing Funders do not see each other's Private Quotes, Coordinators do not read Private Quote contents by default, Sellers see Seller Quote View and funding evidence status/attestation rather than raw proof data or balances, and the designated Auditor sees scoped settlement evidence rather than full RFQ data.
- Negative tests should verify that expired Quotes cannot settle and invalid settlement inputs fail atomically.
...
- The completed implementation is the code-level source of truth; these planning documents provide product rationale and acceptance context. The documented prior art is the MVP Build Spec, Project Brief, glossary, and ADRs.
...
- The implementation should keep product language precise: Funder, not Lender; Receivable Sale, not loan; CIP-56 token settlement, not production money; `ReceivableSaleSettlement` record, not zero-knowledge proof.
...
- The ideal first implementation milestone is a minimal, demoable Daml/Canton workflow that supports the documented happy path and failed-settlement recovery before adding optional stronger privacy mechanisms or production integrations.
